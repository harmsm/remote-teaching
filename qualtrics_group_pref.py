...
__date__ = "2020-09-26"
__usage__ = "qualtrics_group_pref.py qualtrics_csv class_list_file out_file"
...
def create_requested_groups(csv_file,class_list,out_file):
    """
    ...
    for i, group in enumerate(groups):
        for member in group:
            assigned_dict[member] = i

    final_groups = []
    for email in class_df.email:
        try:
            final_groups.append(assigned_dict[email])
        except KeyError:
            final_groups.append(-1)

    class_df["group"] = final_groups
    class_df.to_csv(out_file)
...
def main(argv=None):
    ...
    create_requested_groups(survey_csv,class_list,out_file)
